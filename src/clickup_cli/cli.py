...
agent note:
  Positional IDs also accept --flag aliases, such as --task-id, --query,
  --doc-id, --page-id, --folder-id, --list-id, --comment-id, and --space.
...
Space aliases are generated by lowercasing space names and replacing spaces
with hyphens. Init stores space IDs only; it does not store default list IDs.""",
        epilog="""\
examples:
  clickup init
...
notes:
  Edit the config after init if you want custom aliases or cached list_id
  defaults for list-bound commands.""",
    )
